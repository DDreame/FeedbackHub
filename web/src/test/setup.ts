...
vi.mock('react-i18next', () => {
  const t = (key: string, options?: Record<string, unknown>) => {
    const translations: Record<string, string> = {
      ...
      'thread.eyebrow': '反馈详情',
      'thread.loading': '加载中...',
      'thread.loadError': '加载失败',
      'thread.retry': '重试',
      'thread.noMessages': '暂无消息记录',
      'thread.noFeedback': '未找到反馈记录',
      'thread.developer': '开发者',
      'thread.you': '您',
      'thread.attachmentAlt': '附件 {{index}}',
      'thread.replyPlaceholder': '输入您的回复...',
      'thread.send': '发送',
      'thread.sending': '发送中...',
      'thread.backToHistory': '← 返回反馈列表',
      'thread.sendError': '发送失败',
      'thread.delete': '删除',
